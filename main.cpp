...
int main() {
  unsigned long long kill_memory;
  std::cout << "enter memory top kill in gb: \n";
  std::cin >> kill_memory;

  char *killer = new char[kill_memory * 1024 * 1024 * 1024];
  ...
  std::cin >> step;
  if (work_mode == 'w' && kill_memory > 0 && step > 0) {
  ...
  } else if (work_mode == 'r' && kill_memory > 0 && step > 0) {
    for (int i = 0; i < (kill_memory * 1024 * 1024 / 4); i++) {
